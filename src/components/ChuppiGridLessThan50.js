...
class ChuppiGridLessThan50 extends Component {
  constructor(props) {
    super(props);
    this.state = { input: "" };
  ...
  getDataCustom(ct) {
    ...
    const url = [config.APIURL, "lessthan",ct].join("/");
    console.log(url);
    var { start, end } = this.state.pagination;
    axios
      .get(url)
              ...
              ? this.state.data.map((i, j) => (
                  <div key={j} className="chuppiGridPod">
                    ...
                    <div className="chuppiGridItem">
                      <a className="jo" href={[config.SEARCHURL,"p", i.itemid].join("/")}>
                        {i.itemid}
                      </a>
                    </div>
                    {/* Loading images */}
...
export default ChuppiGridLessThan50;
